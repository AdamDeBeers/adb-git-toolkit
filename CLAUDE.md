...
ADB Git Toolkit (ADB GT) is an open-source Git/GitHub workflow toolkit for Klipper-based 3D printers (Voron and similar), with Mainsail integration via Moonraker's update_manager. The project is in early development (v0.3.0), MIT licensed.

The toolkit is a single interactive Bash script: `scripts/adb-git-toolkit.sh`. It presents a numbered menu (`main_menu`) driving one function per action: repository status, log/history, remote info, diff, create backup (commit), push to GitHub, safe pull, switch branch, quick stash, repository health, configuration restore, check for updates, setup repo files, and about. Running the script with a single action-name argument (e.g. `status`, `backup`, `push`) instead runs that one action non-interactively and exits, skipping the interactive menu -- see the "CLI dispatch" section of `scripts/adb-git-toolkit.sh` and `docs/usage.md`'s "Non-interactive CLI mode" section. There is no build step or package manager. Verification is done both by exercising the menu against real (throwaway) Git repos and by the `tests/adb-git-toolkit.bats` suite (see below).
...
`.github/workflows/tests.yml` installs `bats` and runs `tests/adb-git-toolkit.bats` on push to `main` and on pull requests. Run `bats tests/` locally before pushing changes to `scripts/adb-git-toolkit.sh` — most tests source the script's functions into a throwaway git repo (stripping everything from the `# --- CLI dispatch` marker to EOF, so the trailing arg-parsing block doesn't try to interpret the test harness's own `$1`/`$#`) and drive each menu function by piping canned input to it, the same way the interactive menu receives input. A separate "CLI dispatch" section of tests runs the real, unstripped script directly (`bash "$SCRIPT" <action>`) to exercise `--help`/`--version`/unknown-action/argument dispatch itself. The suite covers the guard clauses (dirty tree, no remote, detached HEAD, etc.), the `[Y/n]` confirmation parsing, and the secrets-warning heuristic in `create_backup`. When adding a new menu action, wire it into both the `main_menu` case statement *and* the CLI dispatch case statement, and add or update a corresponding test. Note: `read -rp "prompt"` never prints its prompt text when stdin isn't a real TTY (piped input, as in every test) — assert on unconditionally-`echo`'d text instead of prompt strings.
...
- **New menu actions**: add a new function following the existing pattern (`header` → guard → work → `pause`), then wire it into the `case` statement inside `main_menu` *and* into the CLI dispatch `case` statement near the bottom of the script (and `print_usage()`'s list) — remember to renumber the trailing `About` entry (currently `14`) if you insert before it.
